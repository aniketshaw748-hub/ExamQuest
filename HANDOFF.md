...
### 2026-06-27 — TUTOR REPORTS -> GITHUB ISSUES (wired + verified live) + full D.Maths answer audit
Two things this session: (1) finished wiring the AI-tutor report system so reports actually reach the owner, and (2) audited every Discrete Mathematics answer for correctness and fixed the high-confidence errors.

**1. Report system now opens GitHub issues.** The report plumbing already existed (`api/report.js` `handleReport`, the `[[REPORT:kind]]` marker protocol in `Chatbot.jsx`, `REPORT_PROTOCOL` in `teachPrompt.js`) but had **no delivery configured**, so prod reports only hit Vercel logs. Fixed:
- **Code gap fixed:** the dev middleware (`vite-gemini.js`) only forwarded `REPORT_WEBHOOK_URL`, never the GitHub vars, so GitHub delivery worked in prod but not dev. Changed `geminiPlugin(apiKey, reportEnv)` to take the full report env and pass it to `handleReport`; `vite.config.js` now passes `{REPORT_WEBHOOK_URL, GITHUB_TOKEN, GITHUB_REPO}`. `api/report.js` now logs GitHub API failures (status + body) instead of silently falling back to `"logged"`.
- **Config:** `GITHUB_REPO=aniketshaw748-hub/ExamQuest` + a `GITHUB_TOKEN` (fine-grained PAT, Issues: Read+write) set in `.env` (local, gitignored) AND in the Vercel `quest` project env (Production+Preview). Reports now create labelled issues (`bug`/`enhancement` + `from-tutor`). Verified end-to-end: dev (`delivered:"github"`) and prod (after redeploy, `delivered:"github"`). Test issues #1/#2 were created and closed.
- Commit `3f7df3e` (code). Note: the Vercel `quest` project is on a team the connected Vercel MCP account can't access (403), so prod env/redeploy were done by the user in the dashboard.

**2. Real bug reports caught.** Once live, the tutor filed two genuine reports as issues: **#3** (shared Gemini key "high demand"/rate-limit, expected free-tier behaviour, BYOK is the fix, closed) and **#4** (Ch3 pigeonhole walkthrough had a wrong answer). Both closed with explanatory comments.

**3. Pigeonhole fix (issue #4).** "12 balls of one colour from 10 blue/20 red/8 green/15 yellow/25 white" was answered **56** (`nk+1 = 5*11+1`), which wrongly assumes every colour has >=11 balls. Blue (10) and green (8) cap their own worst case: `11+11+11+10+8 = 51`, so the **52nd** ball forces 12. Corrected to **52** in `dmaths-walk.js`, `dmaths.json`, the source `.md`, and the `dmaths-teach.js` pigeonhole example (added the "each colour has >=11" caveat there). Commit `778cc88`.

**4. FULL D.MATHS ANSWER AUDIT (299 questions: 115 MCQ + 96 short + 88 long, + curated walks/lessons).** Ran 6 parallel per-chapter agents that recomputed every answer, then **independently re-verified every finding** before changing anything (caught the ch6 agent hallucinating/misattributing several `long` chromatic-polynomial findings — e.g. it claimed a generating-function error in ch6 long#2, which is actually correct). **The "verified" source data had ~33 high-confidence wrong answers (~11%).**
- **28 fixes applied** to `public/data/dmaths.json` via a scripted, asserted batch (each MCQ flip checked its old value first; parse->stringify round-trip is byte-identical so the diff is clean):
  - **16 MCQ answer-key flips:** ch1 #4(c->b), #17(d->a); ch3 #4(a->c), #5(a->c), #9(a->b); ch4 #3(d->a), #5(a->d), #7(d->b), #8(a->b), #9(d->a), #11(a->b), #13(c->b), #14(d->c), #15(d->b); ch6 #13(b->a), #15(b->c). (ch4 had 11/23 keys wrong.)
  - **12 worked-answer rewrites:** ch1 short#1 (481->192), short#2 (454->457); ch2 long#2a Bezout (u=3->u=2), long#3b congruence (mod 104->304, x=82,386,690), short#8 (17->9); ch3 short#12 CNF (4-clause->correct 5-maxterm), short#3 (was "the problem is wrong" -> real R(3,3)=6 proof), short#5 (added the missing union = 245); ch4 short#8 (claimed tautology -> actually false at p=q=r=0); ch5 short#8 (fixed the contradictory S1*A "in S" step -> not a right ideal); ch6 short#1 (chi(K6-ab)=4->5), short#2 (16->972).
- **Left for human review:** see `MATHS-REVIEW-NEEDED.md` (repo root). Includes the broken ch4 mcq#6 (no option equals A->B), diagram-dependent ch6 long chromatic polynomials (#6a, #8b — audit unreliable here), low-confidence/interpretation items (ch3 mcq#7, ch4 mcq#20, ch6 mcq#22/#26/#27, short#9), and the "not divisible by any" interpretation inconsistency (ch1 uses divisible-by-none; ch2 long#3a uses not-all-three -> would be 286 under the other reading).
- **Verified:** `npm run build` green, JSON valid, fixes spot-checked.
- **DURABILITY CAVEAT:** fixes are in `dmaths.json` (deployed) + curated JS, but the `Clg shit/D.Maths/*.md` sources were NOT all updated (only pigeonhole). **Re-parsing D.Maths would revert these fixes** — port them to the `.md` first, or don't re-parse. The other 5 subjects were NOT audited; same audit should be run on them.
