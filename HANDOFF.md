...
- **Authored ladders so far:** D.Maths (all 6 ch, fully bespoke with interactive explainers), **DAA** ch1+ch3, **Automata** ch1+ch2, **CA** ch1+ch2 (Explanation-Ladder lessons via per-subject TEACH registry in `lessons.js`; DAA reuses graph/tree/gmatrix/bigo explainers). **Chemistry** (120 verified concepts → rich fallback) and **EVS** run on the concept-lesson fallback. Remaining non-D.Maths chapters use the fallback (fully functional, just not custom-illustrated).
